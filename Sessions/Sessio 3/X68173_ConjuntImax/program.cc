...
void Cjt_estudiants::afegir_estudiant(const Estudiant& est) {
    ...
    if (imax == -1 and est.te_nota()) imax = i;
        ...
        else if (vest[i].consultar_nota() == vest[imax].consultar_nota() and vest[i].consultar_DNI() < vest[imax].consultar_DNI()){
            imax = i;
        }
    }
...
void Cjt_estudiants::recalcular_posicio_imax(){
    ...
    for (int i = 0; i < nest; ++i){
        if (imax >= 0){
            ...
            else if (vest[i].te_nota() and vest[i].consultar_nota() == vest[imax].consultar_nota()){
